...
function ModalWarn() {
  const { removeActivity, modalState, dispatchModal, handleModal } = useContext(Konteks)

  function deleteItem(e) {
    e.stopPropagation()
    removeActivity(modalState.id).finally(() =>
      dispatchModal({ type: 'DELETE_SUCCESS', message: 'Activity berhasil dihapus' }),
    )
  }
    ...
    <div
      ...
      <div className='flex items-center justify-center w-full h-1/2'>
        <h3 data-cy='modal-delete-title' className='lg:text-base text-center px-12 text-fontColor-900'>
          Apakah anda yakin ingin menghapus Activity <strong>&quot;{modalState.message}&quot;</strong>?
        </h3>
      </div>
      <div className='flex items-center justify-center space-x-2 lg:space-x-3 w-full h-4/5'>
        ...
        <button data-cy='modal-delete-confirm-button' onClick={deleteItem} className={`${btnS} bg-indicator-red text-white`}>
          Hapus
        </button>
      </div>
    </div>
